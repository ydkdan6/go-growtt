...
export default function Login() {
  ...
  const { mutate: signIn, isPending, error } = useSignIn({
    onSuccess: () => {
      // ✅ Sign-in only returns { token, id } — no user fields to display
      toast({ title: "Welcome back!", description: "You have signed in successfully." });
      navigate("/dashboard");
    },
    onError: (msg) => {
      toast({ title: "Login failed", description: getErrorMessage(msg), variant: "destructive" });
    },
  });

  // ─── Google sign-in ─────────────────────────────────────────────────────
  const { mutate: googleSignIn, isPending: isGooglePending } = useGoogleAuth({
    onSuccess: (data) => {
      const displayName = (data as any).full_name || (data as any).username || (data as any).email || "there";
      toast({ title: "Welcome!", description: `Signed in as ${displayName}` });
      const isNew = (data as any).is_new_user ?? !(data as any).financial_literacy_level;
      navigate(isNew ? "/onboarding" : "/dashboard");
    },
    onError: (msg) => {
      toast({ title: "Google sign-in failed", description: getErrorMessage(msg), variant: "destructive" });
    },
  });
  ...
  const handleGoogleSignIn = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      ...
      if (!code) {
        toast({ title: "Google sign-in failed", description: "No authorization code returned.", variant: "destructive" });
        return;
      }
      googleSignIn({ id_token: code });
    },
    onError: () => {
      ...
      <div className="h-full flex flex-col p-10">
        <div className="flex gap-7 mb-8">
          <Link to="/" className="flex-1">
            <AuthButton variant="primary" fullWidth>Login</AuthButton>
          </Link>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8 mb-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Link to="/forgot-password" className="text-[rgba(0,128,128,0.6)] text-base text-right leading-[150%] hover:text-brand-teal transition-colors">
              Forgot Password?
            </Link>
          </div>
          ...
          <AuthButton variant="primary" fullWidth className="mb-10 text-white" type="submit" disabled={isAnyPending}>
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="w-full flex flex-col gap-4 items-center">
            ...
            <p className="text-[rgba(3,7,18,0.8)] text-center text-base leading-[150%]">Or continue with</p>
          </div>

          <div className="flex flex-col md:flex-row gap-4">
            <button type="button" onClick={() => handleGoogleSignIn()} disabled={isAnyPending} className="flex items-center gap-2 hover:opacity-80 transition-opacity disabled:opacity-40">
              {isGooglePending ? (
              ...
              <span className="text-[rgba(3,7,18,0.8)] text-center text-base leading-[150%]">{isGooglePending ? "Signing in…" : "Sign in with Google"}</span>
            </button>

            <button type="button" disabled={isAnyPending} className="flex items-center gap-2 hover:opacity-80 transition-opacity disabled:opacity-40">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              <span className="text-[rgba(3,7,18,0.8)] text-center text-base leading-[150%]">Sign in with Apple</span>
            </button>
          </div>
        </div>
        ...
        <div className="mt-2 pt-2 text-center">
          <p className="text-[rgba(3,7,18,0.6)] text-base leading-[150%]">
            ...
            <Link to="/signup" className="text-brand-teal hover:underline">Sign Up</Link>
          </p>
        </div>
      </div>
    </AuthLayout>
